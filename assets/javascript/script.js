...
function registrarParticipante() {
  ...
  axios.post("http://localhost:5000/participants", dados)
    .then(()=>entrarNaSala())
    .catch(()=>perguntarNome());
}
...
function carregarMensagens() {
  ...
  const header = {headers: {User: nome}}
  axios.get("http://localhost:5000/messages?limit=50", header)
    .then((res)=>processarMensagens(res));
}
...
function carregarParticipantes() {
  ...
  requisicao.then((res)=>processarParticipantes(res));
}
...
function processarMensagens(resposta) {
  mensagens = resposta.data.reverse();
...
function enviarMensagem() {
  ...
  requisicao.catch((err)=>atualizarPagina());
}
